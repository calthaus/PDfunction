...
# "upper limit",e=EC50/inflection point,c=lowerlimit,b=slope
...
plotAxisRange <- function(x, xlab, ylab, main) {
    ...
    plot(fakemodel,type="all",xlim=c(0.00001,0.07), ylim=c(0,110),broken=F,lty="dashed",
         lwd=1.3,col="white",pch=19,cex=1.4,cex.lab=1.3,ylab=ylab, xlab="antibiotic conc [mg/L]")
    axis.break(axis=1,breakpos=0.00002,style="slash") 
...
calculate_anchor <- function(mydata, fctname){
    ...
    modell = M$model
    dataN = M$data
    EC50 = ED(modell,50,display=FALSE)[1]
	print("EC50")
	print(EC50)
    xmax = max(mydata$dose)
    anchtdose <- min(c(EC50*100, xmax*100))  # anchor position
    cat("Anchor:",anchtdose,"\n")
    return(list(anchor=anchtdose, EC50=EC50, xmax=xmax))
}
...
plot_points <- function(mydata,xlab,outname){
    ...
    for(i in experiments){
        ...
        points(expresp ~ expdose, type="p", pch=21, col=colList[i], pt.bg=colList[i],cex = 1.3)
    }
}
...
plot_experiments <- function(einzelkurven,avgmodel,fctname,anchtdose,xlab,plotnr,plotname,outname){
    ...
    for(i in experiments) {
        ...
        plot(singlecurves, type="all", broken=TRUE,add=TRUE,xlim=c(0, max(einzelkurven$dose)*100),
             lty="dashed", lwd=1.3, bg=colList[i], cex.lab=2, cex.main=2, pch=21, cex=1.5,cex.axis=1.5,
             ylab=ylab, xlab=xlab)
    }
    # Plot the whole thing 
    ylab <- "Response (percent of control)"
    plot(avgmodel,type="none",broken=TRUE,add=TRUE, xlim=c(0, max(einzelkurven$dose)),
         lwd=1.5,col="black",cex.lab=1.2,cex.main=2,pch=21,cex=1.2,ylab=ylab,xlab=xlab)
    if (!is.null(anchtdose)) {
        points(anchtdose,0,type="p",pch=17, col = "white", cex = 2)
    }
}
...
calculate_BMC <- function(avgmodel){
    ...
    shapepar=round(coefficients(avgmodel)[1],5)
    print(avgmodel)
    print("*********************************model******************************")
    print(avgmodel)
    BMCdf <- data.frame(rbind(as.data.frame(BMRs)), BMC[,c("Estimate","Lower","Upper")],shapepar)
    colnames(BMCdf) <- c("BMR", "BMC", "BMCL", "BMCU","shapepar")
    rownames(BMCdf) <- NULL
    return(BMCdf)
}
...
plot_bmc <- function(BMCdf){
    BMR = 99
    BMC99 =BMCdf[BMCdf$BMR==BMR,]
    shape = BMCdf$shapepar
    EC99 = round(BMC99$BMC,2)
    BMCL = BMC99$BMCL
    BMCU = round(BMC99$BMCU,2)
    ECround <- round(EC99,6)
    abline(v=0.002563538, col="grey")
    abline(v=0.001452514, col="black")
    #abline(h=NA,x1=0.001,x2=ECround,v=ECround,y1=Null,lty=1,col="black")
    #ablineclip(h=1000,x1=0.00000001,x2=BMCL,v=BMCL,y1=-20,lty=3,col="grey")

}
#    legend("bottomleft",
#        c(paste("MIC99=",EC99),paste("MICU99=",BMCU),lgshape),
...
# Writes a log file with columns BMR,BMC,BMCL,BMCU,b,c,d,e,anchor,fitname,samplename
write_bmc <- function(BMCdf, avgmodel, fctname, anchtdose, outname, plotnrs){
    ...
    write.table(
        cbind(
            ...
            t(estimates),
            data.frame(fctname=fctname,outname=outname)
        ),
        file=dateiname,
        append=TRUE,
...
processData <- function(mydata, outname, xlab, plotname, run=4, plotnr=0, nsteps=0, figures=TRUE) {
    ...
    if (is.null(mlist) || run==1) {
        kurvenbild <- makepng(paste(plotname,.pdf))
    ...
    ## Set plot options
    if (figures) {
        #kurvenbild <- makepng(paste(plotname,".pdf"),width = 10, height = 10)
      kurvenbild <- makepng(paste(plotname,".pdf"))
        #par(mar=c(5.1, 10.1, 10.1, 10.2), xpd=TRUE)
        ylab <- "Viability [% of untreated]"
        plotAxisRange(mydata$dose, xlab, ylab, outname)
    }

    if (run==2 && figures){
        plot_experiments(dataP,modelP,fctname,anchtdose=NULL,xlab,plotnr,plotname,outname)
    } else if (run>=3) {
        # Anchor calculation
        A = calculate_anchor(mydata, fctname)
        if (plotnr==0){  # Standard, as in the paper - not the simulation
            anchtdose = A$anchor
            plotnrs = 0
        # Simulation stuff
        } else if (plotnr>0){
            ...
            interval <- seq(from=(EC50*1),to=(EC50*400),length.out=400)
        ...
        if (run>=4 && !is.null(avgmodel)) {
            ...
            if (figures) {
              ...
              legend("topright", 1.9 ,c("experiment 1","experiment 2","experiment 3"), pt.bg = c("red","orange","blue"),
              	         text.col = "black", lty = c(1), pch = 21, col="black",bty="n",cex=0.75,
                         merge = TRUE, bg = "white")
                # Add the vertical lines to the plot

                #plot_bmc(BMCdf)
                # Add the table at the bottom with the BMC values
#                 bmctable <- as.table(as.matrix(round(BMCdf,4)))
