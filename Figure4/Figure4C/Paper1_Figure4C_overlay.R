...
data<-readWorksheetFromFile("C:/Users/Sunny/Dropbox/Time_kill/ClassicalTK/singleplot_statistics_WT.xlsx", sheet = 1, header = TRUE, startCol = 0, startRow = 0)
par(mar=c(4.7,5,2,7))
par(oma=c(0,0,0,0))
...
curve(psi(data$upper[m],data$lower[m],data$kappa[m],data$zMIC[m],x),1e-10,100000,add=TRUE,col=color[m],lwd=3)
m=m+1
print(m)
}
