...
for(j in unique(mydata$replicate)){
  ...
  secondf=subset(slopes,slopes$dose>=0.0002)model=drm(firstdf$slope~firstdf$dose,fct=LL.4())
  model2=drm(secondf$slope~secondf$dose,fct=LL.4())
  #plot(model2,xlim=c(0,1))
  df=slopes$dose
  #start parameters
  ...
  psimin1 <- coefficients(model)[2]
  psimax1 <- coefficients(model)[3]
  slope1 <- coefficients(model)[1]
  flex1 <- coefficients(model)[4]
  
  psimin2 <- coefficients(model2)[2]
  psimax2 <- coefficients(model2)[3]
  slope2 <- coefficients(model2)[1]
  flex2 <- coefficients(model2)[4]
  
  #spielwiese
  dev.off()
  plot(model,xlim=c(0,1),ylim=c(-5,2))
  plot(model2,add=TRUE)
  zweiphasic<-function(b,c,d,e,b1,c1,d1,e1,x) {
    (c +
       (d-c)/
       (1+ exp(b*(log(x)-log(e))))) *
      (c1 +
  ...
  x=df
  #curve(zweiphasic( 1.95234614,-0.67667978, 0.90417939, 0.00008599,2.124268 ,-2.398481, -0.544558 , 0.005729,x),2e-7,1,add=TRUE,col="black",lty=1,lwd=2)
  curve(zweiphasic( slope1 ,psimin1, psimax1 , flex1, slope2 ,psimin2, psimax2 , flex2, x),2e-7,1,add=TRUE,col="red",lty=1,lwd=2)
  curve(-zweiphasic( slope1 ,psimin1, psimax1 , flex1, slope2 ,psimin2, psimax2 , flex2, x),2e-7,1,add=TRUE,col="green",lty=1,lwd=2)
