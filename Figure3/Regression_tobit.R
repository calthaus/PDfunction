...
for(j in unique(mydata$replicate)){
  ...
  for(idose in 1:nlevels(ab.conc)){
    ...
    #geomean=log(CFU[hours==0 &antibiotic==dose])
    ...
    #fit <- mle2(nll, start=list(a =log(1000000), b=b,sigma=0.1), method="Nelder-Mead", control=list(maxit=1e3))
    #predict<-coef(fit)[[1]]+data2regress$x*coef(fit)[[2]]
    #curve(exp(coef(fit)[1])*exp(coef(fit)[2]*x),0,6,add=TRUE,col=mypalette[idose],lty=2)
    #curve(exp(coef(tobit)[1])*exp(coef(tobit)[2]*x),0,6,add=TRUE,col=mypalette[idose],lty=2)    
